...
public class WebServer {
  ...
  /**
   * The URL pattern to render the Home page.
   */
  public static final String HOME_URL = "/";

  /**
   * The URL pattern to render the Sign in page.
   */
  ...
  /**
   * The URL pattern to render the game page.
   */
  ...
  public static final String ENDSPECTATE_URL = "/spectator/stopWatching";
  ...
  public void initialize() {
    ...
    // show signout
    post(SIGN_OUT_URL, new PostSignOutRoute(templateEngine,gameCenter,playerLobby));
    ...
    post(SUBMIT_TURN_URL, new PostMakeMoveRoute(templateEngine, gson, playerLobby, gameCenter));

    //post backup move request
    post(BACKUP_MOVE_URL, new PostRevokeMoveRoute(gson, gameCenter));

    //Post check turn request
    post(CHECK_TURN_URL, new PostCheckTurnRoute(gson, gameCenter));

    //post resign game request
    post(RESIGN_GAME_URL, new PostResignGameRoute(templateEngine, gson, playerLobby, gameCenter));

    //get Spectator view request
    get(SPECTATE_URL, new GetSpectatorViewRoute(playerLobby, gameCenter));
    ...
    //post end spectate request
    post(ENDSPECTATE_URL, new PostEndSpectateRoute(gameCenter));
