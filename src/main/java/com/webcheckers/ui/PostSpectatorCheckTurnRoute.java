...
public class PostSpectatorCheckTurnRoute implements Route {
    ...
    public Object handle(Request request, Response response) throws Exception {
        ...
        // Get the game
        Game spectatingGame = gameCenter.getGame(spectator.getName());
        ...
        if(spectatingGame.gameOver()) {
            ...
            Piece.pieceColor currentTurn = httpSession.attribute(PostSpectatorCheckTurnRoute.TURN_KEY);

            if(currentTurn == spectatingGame.getActiveColor()) {
                turnResult = Message.info(String.format("It is still %s's turn", (currentTurn == Piece.pieceColor.RED) ? spectatingGame.getRedPlayerName() : spectatingGame.getWhitePlayerName()));
            }
