...
public class MoveValidationHelper {
     ...
     */
    public int piecesOfColor(BoardView board, Piece.pieceColor color) {
        int pieces = 0;
        //iterate through each space
        for (Row row: board) {
            for (Space space: row) {
                Piece piece = space.getPiece();
                Piece.pieceColor pieceColor = null;
     ...
     */
    public void scanBoard(BoardView board, Piece.pieceColor activeColor, ArrayList<Move> possibleMoves,
     ...
     */
    private void initMoves(Position start, BoardView board, Piece.pieceColor activeColor, Piece.Type pieceType, ArrayList<Move> possibleMoves, ArrayList<ArrayList<Move>> possibleJumps){
     ...
     * @param activeColor active player's piece color
     * @param possibleJumps the list of move sequences
     * @return : true if any moves were added to a sequence, false if no moves were added
     */
    private boolean addMoreJumps(BoardView board, Piece.pieceColor activeColor,
                                 ArrayList<ArrayList<Move>> possibleJumps) {
        ...
        for (int i = 0; i < possibleJumps.size(); i++) {
            ...
            if (newJumps) {
                added_flag = true; //if you add anything, do another loop through to double-check
            }
        }
        return added_flag;
    }
     ...
     */
    public Move checkValidJump(Position start, Position middle, Position end, BoardView board, Piece.pieceColor activeColor){
     ...
     * @param position the posiiton to check for piece
    ...
    /**
     * Checks for valid non jump move given the start and end
     * @param start the starting position for the move
     * @param end the end position for the move
     * @param board the board to check for the move
    ...
    /**
     * Adds any possible valid jump into the move provided (the last move in the move sequence)
     ...
     */
    private boolean addAJump(Move move, BoardView board, Piece.pieceColor activeColor, Piece.Type pieceType, ArrayList<ArrayList<Move>> possibleJumps, int moveListIndex){
